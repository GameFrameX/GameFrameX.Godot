...
namespace GameFrameX.Entity.Runtime
{
    ...
    public partial class DefaultEntityHelper : EntityHelperBase
    {
        ...
        public override object InstantiateEntity(object entityAsset)
        {
            ...
            if (entityAsset is Node node)
            {
                return node.Duplicate();
            }

            Log.Error("Entity asset type is invalid.");
            return null;
        }
        ...
        {
            if (entityInstance is not Node entityNode)
            {
                Log.Error("Entity instance is invalid.");
                return null;
            }

            if (entityGroup?.Helper is Node groupNode)
            {
                groupNode.AddChild(entityNode);
            }
            else
            {
                Log.Error("Entity group helper is invalid.");
                return null;
            }
