...
namespace FairyGUI.Utils
{
    ...
    public class HtmlInput : IHtmlObject
    {
        ...
        public static Color defaultBackgroundColor = Colors.Transparent;

        public HtmlInput()
        {
            textInput = (GTextInput)UIObjectFactory.NewObject(ObjectType.InputText);
            textInput.name = "HtmlInput";
        ...
        {
            if (!_hidden)
                _owner.AddChild(textInput.displayObject.node);
        }

        public void Remove()
        {
            if (!_hidden && textInput.displayObject.parent != null)
                _owner.RemoveChild(textInput.displayObject.node);
        }
