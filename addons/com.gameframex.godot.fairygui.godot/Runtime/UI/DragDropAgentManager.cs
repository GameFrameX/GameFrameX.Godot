namespace FairyGUI
{
    ...
    /// 这是一个提供特殊拖放功能的功能类（对应 Unity 侧 com.gameframex.unity.fairygui.unity 的 DragDropAgentManager）。
    /// 与 GObject.draggable 不同，拖动开始后，他使用一个替代的图标作为拖动对象。
    /// 当玩家释放鼠标/手指，目标组件会发出一个 onDrop 事件。
    /// </summary>
    public sealed class DragDropAgentManager
    {
        private const string DropEventName = "onDrop";
        ...
        {
            get { return _agent != null && _agent.parent != null; }
        }
        ...
        public void StartDrag(GObject source, object sourceData, EventCallback1 onDragEndCallback = null, int touchPointID = -1, float scale = 1.5f)
        {
            ...
            source.onDragEnd.Add(() =>
            {
                ...
                while (obj != null)
                {
                    if (obj.hasEventListeners(DropEventName))
                    {
                        obj.RequestFocus();
                        obj.DispatchEvent(DropEventName, sourceData, source);
                        return;
                    }
