...
{
    /// <summary>
    /// 资源系统核心入口（包管理、运行时调度、基础加载API）。
    /// 说明：增量更新流程由业务层服务封装；Godot资源/PCK桥接由 AssetSystemExtension 提供。
    /// </summary>
    [AssetSystemPreserve]
    public static partial class AssetSystem
    {
        ...
        public static void Initialize(ILogger logger = null)
        {
            if (_isInitialize)
            {
                AssetSystemLogger.Warning($"{nameof(AssetSystem)} is initialized !");
                return;
            }

            if (_isInitialize == false)
            {
                AssetSystemLogger.Logger = logger;
                ...
                if (_autoCreateDriver)
                {
                    ...
                    {
                        AssetSystemLogger.Warning("Can not create Godot driver node. Please call AssetSystem.Tick() manually before Godot SceneTree is ready.");
                    }
                }
                AssetSystemLogger.Log($"{nameof(AssetSystem)} initialize !");
        ...
        public static ResourcePackage CreatePackage(string packageName)
        {
            ...
            AssetSystemLogger.Log($"Create resource package : {packageName}");
        ...
        public static ResourcePackage GetPackage(string packageName)
        {
            ...
            if (package == null)
            {
                AssetSystemLogger.Error($"Can not found resource package : {packageName}");
            }
        ...
        public static bool RemovePackage(string packageName)
        {
            ...
            {
                AssetSystemLogger.Error($"The resource package {packageName} has not been destroyed, please call the method {nameof(ResourcePackage.DestroyAsync)} to destroy!");
                return false;
            }
            ...
            AssetSystemLogger.Log($"Remove resource package : {packageName}");
        ...
        private static bool TryCreateGodotDriver()
        {
            ...
            var driverName = $"[{nameof(AssetSystem)}]";
        ...
        private static void CheckException(string packageName)
        {
            if (_isInitialize == false)
            {
                throw new Exception($"{nameof(AssetSystem)} not initialize !");
            }
        ...
        /// </summary>
        [AssetSystemPreserve]
        public static void SetDownloadSystemWebRequestFactory(WebRequestFactory createDelegate)
        {
            DownloadSystemHelper.RequestFactory = createDelegate;
        }
        ...
        public static void SetOperationSystemMaxTimeSlice(long milliseconds)
        {
            ...
            {
                milliseconds = 10;
                AssetSystemLogger.Warning($"MaxTimeSlice minimum value is 10 milliseconds.");
            }
        ...
        public static void SetAutoCreateDriver(bool autoCreateDriver)
        {
            if (_isInitialize)
            {
                AssetSystemLogger.Warning($"{nameof(SetAutoCreateDriver)} should be called before {nameof(Initialize)}.");
                return;
            }
        ...
        public static bool TryExecuteDebugCommand(string command, string commandParam, out DebugReport report, out string message)
        {
            ...
            if (!_isInitialize)
            {
                message = $"{nameof(AssetSystem)} not initialize !";
                return false;
            }
    ...
    [AssetSystemPreserve]
    internal partial class AssetSystemGodotDriver : Node
    {
        /// <summary>
        /// Godot帧驱动更新入口
        /// </summary>
