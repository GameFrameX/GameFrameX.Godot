...
namespace GameFrameX.Event.Runtime
{
    /// <summary>
    /// 事件管理器接口。
    /// 继承核心层 <see cref="IEventPublisher"/> 契约，供核心组件（BaseComponent）依赖倒置调用。
    /// </summary>
    public interface IEventManager : IEventPublisher
    {
        /// <summary>
        /// 获取事件处理函数的数量。
        /// </summary>
        ...
        // Fire / FireNow 契约由基接口 IEventPublisher（GameFrameX.Runtime）提供，此处不再重复声明。
    }
}