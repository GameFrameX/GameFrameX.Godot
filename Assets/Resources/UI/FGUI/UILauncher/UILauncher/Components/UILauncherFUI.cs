...
namespace Hotfix.UI
{
	public sealed partial class UILauncherFUI : FUI
	{
		...
		public static UILauncherFUI CreateInstance()
		{
			return Create(CreateGObject());
		}

		public static UniTask<UILauncherFUI> CreateInstanceAsync(Entity domain)
		{
			UniTaskCompletionSource<UILauncherFUI> tcs = new UniTaskCompletionSource<UILauncherFUI>();
		...
		public static UILauncherFUI Create(GObject go)
		{
			var fui = go.displayObject.gameObject.GetOrAddComponent<UILauncherFUI>();
			fui?.SetGObject(go);
			fui?.InitView();
			return fui;
		}
		...
		/// </summary>
		public static UILauncherFUI GetFormPool(GObject go)
		{
			var fui = go.Get<UILauncherFUI>();
			if (fui == null)
			{
				fui = Create(go);
			}
		...
		private UILauncherFUI(GObject gObject) : base(gObject)
		{
			// Awake(gObject);
		}
	}
